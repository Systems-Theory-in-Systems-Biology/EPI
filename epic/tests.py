...
def KDETest1DataPoint():
    ...
    surf = ax.plot_surface(xMesh, yMesh, gaussEvals, alpha = 0.75, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    plt.show()
    
    fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
    surf = ax.plot_surface(xMesh, yMesh, cauchyEvals, alpha = 0.75, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    plt.show()  
...
def transformationTestLinear():
    ...
    fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
    plt.title("Data KDE")
    surf = ax.plot_surface(KDExMesh, KDEyMesh, gaussEvals, alpha = 0.75, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    plt.show()
    ...
    for i in range(paramResolution):
        for j in range(paramResolution):
            ...
            paramEvals[i,j], _ = evalLogTransformedDensity(paramPoint, modelName, data, dataStdevs)

    paramEvals = np.exp(paramEvals)
    
    fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
    plt.title("Paramter Density Estimation")
    surf = ax.plot_surface(paramxMesh, paramyMesh, paramEvals, alpha = 0.75, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    plt.show()
