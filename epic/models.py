...
def returnVisualizationGrid(modelName, resolution):
    ...
    np.savetxt("Applications/" + modelName + "/Plots/dataGrid.csv", dataGrid, delimiter = ",")
    np.savetxt("Applications/" + modelName + "/Plots/paramGrid.csv", paramGrid, delimiter = ",")
...
def calcStdevs(data):   
    ...
    for i in range(dataDim):
        stdevs[i] = np.sqrt(np.sum(np.power(data[:,i]-means[i],2))/(numDataPoints-1))
        
    maxStderiv = np.amax(stdevs)
    
    # Silvermans rule
    return stdevs*(numDataPoints*(dataDim+2)/4.0)**(-1.0/(dataDim+4))

## loads and returns all data for a chosen model
# Input: modelName (model ID)
# Output: paramDim (number of model parameters)
...
def dataLoader(modelName):
    ...
    centralParamsDict = {
        ...
        "Stock": np.array([0.41406223, 1.04680993, 1.21173553, 0.8078955,1.07772437,0.64869251]),
        "StockArtificial": np.array([0.41406223, 1.04680993, 1.21173553, 0.8078955,1.07772437,0.64869251])}
...
# Input: modelName (model ID)
# Output: params (true parameters used to generate artificial data)
...
def generateArtificialTemperatureData():
    ...
    rawTrueParamSample = np.loadtxt("Data/TemperatureArtificialParams.csv", delimiter = ",")
    ...
    np.savetxt("Data/TemperatureArtificialData.csv", artificialData, delimiter = ",")
...
def generateArtificialCoronaData():
    ...
    for j in range(numSamples):
        trueParamSample[j,:] = lowerBound + (upperBound-lowerBound)*trueParamSample[j,:]
        artificialData[j,:] = coronaModel(trueParamSample[j,:])

    np.savetxt("Data/CoronaArtificialData.csv", artificialData, delimiter = ",")
    np.savetxt("Data/CoronaArtificialParams.csv", trueParamSample, delimiter = ",")
...
def generateArtificialStockData():
    ...
    mean = np.array([0.41406223, 1.04680993, 1.21173553, 0.8078955,1.07772437,0.64869251])
    ...
    np.savetxt("Data/StockArtificialParams.csv", trueParamSample, delimiter = ",")
...
def generateStockData(tickerList, tickerListName):
    ...
    dates = np.array(["2022-01-31","2022-02-01","2022-02-02","2022-02-03","2022-02-04",
    ...
    for i in range(stocks.shape[0]):
        try:
            ...
            try:
                ...
                stockData[successCounter,:] = stockData[successCounter,:]-stockData[successCounter,0]
    ...
    np.savetxt("Data/DataOrigins/Stock/" + tickerListName + "Data.csv", stockData[0:successCounter,1:],delimiter = ",")
    np.savetxt("Data/DataOrigins/Stock/" + tickerListName + "IDs.csv", stockIDs, delimiter = ",",fmt ='% s')
...
def coronaModel(logParam):
    ...
    rhs = lambda t,x,param: jnp.array([-param[0]*x[0]*x[2], param[0]*x[0]*x[2] - param[1]*x[1], param[1]*x[1] - param[2]*x[2], param[2]*x[2]])
    ...
    try: 
        odeSol = dx.diffeqsolve(term, solver, t0=0.0, t1=15.0, dt0=0.1, y0=xInit, args=param,saveat=saveat, stepsize_controller=stepsize_controller)     
...
def stockModel(param):
    ...
    def iteration(x,param):
        return jnp.array([param[0]/(1+jnp.power(x[0],2)) + param[1]*x[1] + param[2],
    ...
    timeCourse = jnp.array([x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19])
...
def expModel(param):
    return jnp.array([param[0]*jnp.exp(1), 
                      jnp.exp(param[1])])

# very simple linear ODE test model a*e^(bt)
def linODEModel(param):
    return jnp.array([param[0]*jnp.exp(param[1]*1.0), 
