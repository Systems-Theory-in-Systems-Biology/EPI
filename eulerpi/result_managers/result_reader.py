...
class ResultReader:
    ...
    def load_inference_results(
        ...
        Args:
            num_burn_in_samples(int, optional): Number of samples that will be ignored per chain (i.e. walker). Only for sampling inference. Default is None and uses the value that was used for the inference stored in inference_information.json.
            thinning_factor(int, optional): Thinning factor that will be used to thin the Markov chain. Only for sampling inference. Default is None and uses the value that was used for the inference stored in each inference_information.json.
        ...
        if num_burn_in_samples is not None or thinning_factor is not None:
            logger.info(
                f"For inference type {self.inference_information['inference_type']}, num_burn_in_samples and thinning_factor are ignored."
            )

        return self.load_grid_based_inference_results()

    def load_sampling_inference_results(
        self,
        inference_information: Dict,
        num_burn_in_samples: Optional[int] = None,
        thinning_factor: Optional[int] = None,
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Load the inference results generated by EPI using sampling.
        ...
        num_steps_per_sub_run = inference_information["num_steps_per_sub_run"]
        ...
        for i in range(self.path_manager.count_emcee_sub_runs()):
            ...
            if i == 0:
                ...
                params = params_current_chain.reshape(
                    num_steps_per_sub_run, num_walkers, param_dim
                )
                pushforward_evals = pushforward_evals_current_chain.reshape(
                    num_steps_per_sub_run, num_walkers, data_dim
                )
                density_evals = density_evals_current_chain.reshape(
                    num_steps_per_sub_run, num_walkers, 1
                )
            else:
