Temperature Model
-----------------
The temperature model is contained in :file:`epipy/examples/temperature/temperature.py`.
...
* Additional fixed parameters: The model includes fixed parameters :code:`self.low_T=30.0` and :code:`self.high_T=30.0`.
  ...
  The best way to seperate the outputs for the parametrized models is to pass a string based on the fixed_params to the attribute :py:attr:`run_name` of the :py:func:`~epipy.core.inference.inference` function.
...
.. note::
  ...
  The functions :py:meth:`~epipy.examples.temperature.temperature.TemperatureWithFixedParams.calc_forward` is not strictly necessary.
